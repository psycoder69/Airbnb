...
const enableButton = (button, buttonIcon, buttonText) => {
...
for (let i = 0; i < roomVisibility.length; i ++) {
    roomVisibility[i].onclick = async () => {
        disableButton(roomVisibility[i]);
        ...
        if (hidden) {
            enableButton(roomVisibility[i], `<i class="fa-solid fa-eye-slash"></i>`, `Hide`);
        } else {
            enableButton(roomVisibility[i], `<i class="fa-solid fa-eye"></i>`, `Show`);
        }
    };
}
...
confirmDeleteRoom.onclick = async () => {
    disableButton(confirmDeleteRoom);
    ...
    enableButton(confirmDeleteRoom, `<i class="fa-solid fa-trash-can"></i>`, `Delete`);
